...
static void holdInstallerMutex() {
    ...
    if (!h) h = ::CreateMutexW(nullptr, FALSE, L"ProlineMarketsTerminal.SingleInstance");
}
#endif
...
int main(int argc, char* argv[]) {
    ...
    app.setApplicationName("Proline Markets Terminal");
    ...
    app.setWindowIcon(QIcon(":/prolinemarket-256.png"));
    ...
    MainWindow w(cfg);
    // Maximised, not the 1360x840 the constructor sizes to. This is a trading
    // terminal: Market Watch, four chart panes and the blotter all want room,
    // and every trader was maximising it by hand on every launch. The window
    // still knows its restored size, so the maximise button toggles back to it.
    w.showMaximized();
    return app.exec();
}
