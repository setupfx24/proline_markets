 ...
 *  Server:  INTERNAL_API_URL (Docker hostname) â†’ NEXT_PUBLIC_API_URL â†’ fallback localhost.
 */
export function getApiBase(): string {
  if (typeof window !== 'undefined') {
    // Direct gateway call (fast â€” skips Next.js proxy hop).
...
class ApiClient {
  ...
  clearToken() {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        localStorage.removeItem('ProlineMarkets-auth');
      } catch {
        /* ignore */
      }
    }
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new Error(
        aborted
          ? 'Request timed out â€” start the API gateway (port 8000): docker compose up -d'
          : e instanceof Error
            ? e.message === 'Failed to fetch'
              ? 'Cannot reach API â€” start gateway on port 8000, then refresh.'
              : e.message
            : 'Network error',
      );
    }
