...
QString Config::filePath() {
    ...
    if (dir.isEmpty())
        dir = QDir::homePath() + "/.prolinemarket-terminal";
    QDir().mkpath(dir);
...
//
// The original Bull4x → SwissCresta rename did carry its config forward, because
// that was a rename of one product: the same account, the same backend, only the
// host moved. Every fork since — SwissCresta → TuskaEx → Proline Markets — is a
// *different platform*. A token or API key minted by any of them authenticates
// nothing against api.prolinemarket.com, and adopting one would also drag its
// endpoints in, pointing this build at another broker's API. So a Proline
// Markets install starts with a clean config and a real sign-in.
//
// For the same reason nothing deletes those files either: the older terminals
// may still be installed on this machine and their config is their own.
