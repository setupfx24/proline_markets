/*
 * Proline Markets Broker API adapter for the TradingView Trading Terminal.
 *
...
(function () {
  ...
  function ProlineMarketsBroker(host, bridge) {
    this._host = host;
    this._bridge = bridge;
  ...
  ProlineMarketsBroker.prototype._tick = function (symbol) {
    var m = this._meta[symbol];
    return tickForDigits(m ? m.digits : 5);
  };

  ProlineMarketsBroker.prototype._parsePositions = function () {
    var arr = [];
  ...
  ProlineMarketsBroker.prototype._toTv = function (p) {
  ...
  ProlineMarketsBroker.prototype._reconcile = function () {
  ...
  ProlineMarketsBroker.prototype._updatePL = function (sym) {
  ...
  ProlineMarketsBroker.prototype._recomputeTotals = function () {
    var total = 0;
    var self = this;
    Object.keys(this._positions).forEach(function (id) {
      var r = self._positions[id];
  ...
  ProlineMarketsBroker.prototype._onOp = function (id, op, ok, msg) {
    var key = id + "|" + op;
  ...
  ProlineMarketsBroker.prototype.connectionStatus = function () { return Connected; };

  ProlineMarketsBroker.prototype.chartContextMenuActions = function (context, options) {
    return this._host.defaultContextMenuActions(context, options);
  };

  ProlineMarketsBroker.prototype.isTradable = function (symbol) {
    return Promise.resolve(true);
  };

  ProlineMarketsBroker.prototype.orders = function () { return Promise.resolve([]); };

  ProlineMarketsBroker.prototype.executions = function (symbol) { return Promise.resolve([]); };

  ProlineMarketsBroker.prototype.positions = function () {
    var self = this;
    return Promise.resolve(this._parsePositions().map(function (p) { return self._toTv(p); }));
  };

  ProlineMarketsBroker.prototype.symbolInfo = function (symbol) {
    var m = this._meta[symbol] || {};
  ...
  ProlineMarketsBroker.prototype.accountManagerInfo = function () {
    return {
      accountTitle: "Proline Markets",
      summary: [
  ...
  ProlineMarketsBroker.prototype.accountsMetainfo = function () {
    return Promise.resolve([{ id: "main", name: "Proline Markets" }]);
  };

  ProlineMarketsBroker.prototype.currentAccount = function () { return "main"; };

  // ---- IBrokerTerminal ----------------------------------------------------

  // Order entry stays in the native Order Ticket; reject any chart order entry.
  ProlineMarketsBroker.prototype.placeOrder = function (order) {
  ...
  ProlineMarketsBroker.prototype.modifyOrder = function () { return Promise.resolve(); };
  ProlineMarketsBroker.prototype.cancelOrder = function () { return Promise.resolve(); };

  // Drag SL/TP handle -> modify the real server position.
  ProlineMarketsBroker.prototype.editPositionBrackets = function (positionId, brackets) {
  ...
  ProlineMarketsBroker.prototype.closePosition = function (positionId) {
    var id = String(positionId);
    var self = this;
    return new Promise(function (resolve, reject) {
  ...
  window.PROLINEMARKET_BROKER_CONFIG = {
    configFlags: {
  ...
  window.makeBroker = function (host, bridge) { return new ProlineMarketsBroker(host, bridge); };
})();
