# Proline Markets Terminal

A native **C++ / Qt6 desktop trading terminal** for the Proline Markets platform.
It connects to the [Proline Markets Algo API](../ALGO_API.md) over REST + WebSocket
...
On first launch you sign in with your **Proline Markets email and password** — the
...
your Proline Markets dashboard instead. For production leave the endpoints as the
defaults:

- REST base: `https://api.prolinemarket.com/api/algo`
- WebSocket: `wss://api.prolinemarket.com/ws/algo/prices`

The WebSocket must use the `api.` host — `trade.prolinemarket.com` proxies REST but
...
> This build was forked from the SwissCresta → TuskaEx terminal and shares no
> identity with either: different config path, different installer AppId, and
> **no** config migration. A SwissCresta or TuskaEx sign-in is not carried over
> — those accounts do not exist on the Proline Markets backend. All three can be
